...
class AttentationContextModel:
    ...
    def build_model(self,
        ...
        with tf.device(config.machine):
            ...
            cell_fw = tf.contrib.rnn.LSTMCell(150)
            cell_bw = tf.contrib.rnn.LSTMCell(150)
            (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(cell_fw,
                                                                        cell_bw,
                                                                        context_word_embeddings,
                                                                        dtype=tf.float32)
            output = tf.concat([output_fw, output_bw], axis=-1)
            context_word_embeddings = tf.nn.dropout(output, dropout_prob)
